# coding=utf-8
"""
Async HTTP Client Library Adapters
...
import aiohttp
from async_hvac import exceptions, utils
...
class Adapter(object):
    ...
    async def login(self, url, use_token=True, **kwargs):
        ...
        :rtype: requests.Response
        """
        response = await (await self.post(url, **kwargs)).json()
...
class Request(Adapter):
    ...
    def __init__(self, base_uri=DEFAULT_BASE_URI, token=None, cert=None, verify=True, timeout=30, proxies=None,
                 allow_redirects=True, session=None, namespace=None, loop=None):
        """Create a new request adapter instance.
